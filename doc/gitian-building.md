...
*Setup instructions for a Gitian build of SmartCoin Core using a Debian VM or physical system.*
...
- [Building SmartCoin Core](#building-smartcoin-core)
...
in the smartcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building SmartCoin Core
----------------

To build SmartCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the smartcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/smartcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/smart-coin/SCC
...
For example:
```bash
URL=https://github.com/thrasher-/smartcoin.git
...
and the smartcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../smartcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/smartcoin
SIGPATH=/some/root/path/smartcoin-detached-sigs

./bin/gbuild --url smartcoin=${BTCPATH},signature=${SIGPATH} ../smartcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
[smartcoin-project/gitian.sigs.ltc](https://github.com/smartcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
